(function(){
	...
	function updateDashboard(data){ if(!data) return; const isDisconnected = (data.vMeas === 0 && data.current === 0); if(typeof data.socCc==='number' && !isDisconnected){ socValEl && (socValEl.textContent = `${data.socCc.toFixed(0)}%`); drawSocGauge(socGauge, data.socCc, false); socCcEl && (socCcEl.textContent = `${data.socCc.toFixed(0)}%`); } else if(isDisconnected){ socValEl && (socValEl.textContent = 'Disconnected'); drawSocGauge(socGauge, 0, true); socCcEl && (socCcEl.textContent = '--'); } if(typeof data.socModel==='number' && !isDisconnected){ socModelEl && (socModelEl.textContent = `${data.socModel.toFixed(0)}%`); } else if(isDisconnected){ socModelEl && (socModelEl.textContent = '--'); } const isIdle = (String(data.status||'').toLowerCase().includes('idle')); if(typeof data.vMeas==='number'){ const displayV = (isIdle && data.vMeas===0) ? undefined : data.vMeas; if(displayV!==undefined){ voltValEl && (voltValEl.textContent = displayV.toFixed(3)); history.voltage.push(displayV); if(history.voltage.length>historyLimit) history.voltage.shift(); } } if(typeof data.vModel==='number'){ voltModelValEl && (voltModelValEl.textContent = data.vModel.toFixed(3)); } if(typeof data.current==='number'){ currValEl && (currValEl.textContent = data.current.toFixed(3)); history.current.push(data.current); if(history.current.length>historyLimit) history.current.shift(); } if(typeof data.temp==='number'){ tempValEl && (tempValEl.textContent = data.temp.toFixed(2)); history.temperature.push(data.temp); if(history.temperature.length>historyLimit) history.temperature.shift(); drawSparkline(tempSpark, history.temperature, '#f97316'); } if(isDisconnected){ stateValEl && (stateValEl.textContent = 'Battery Disconnected'); stateValEl && (stateValEl.className = 'state-idle'); } else if(typeof data.status==='string'){ stateValEl && (stateValEl.textContent = data.status || '--'); applyStateColor(data.status); } if(typeof data.timestamp==='string'){ tsValEl && (tsValEl.textContent = data.timestamp || '--'); } }
